...
**「鍵があること」ではなく「実際に動いていること」を見る**: 自動処理は `job_runs` の最終成功、バックアップは48時間以内に
取れているか、運営アカウントはデモ以外が居るか、サイトURLは公開URLか。検索エンジンへの公開も、デモモードが無効かつ
本番キーが入るまでは `robots.ts` が `noindex` を返す（準備中のデモデータが検索結果に出ないように）。
