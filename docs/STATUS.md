...
コード側の残りは、判断待ちの `owner-decision`（#18）と、配送業者との API 契約が決まったあとのつなぎ込み（#25 の続き）。
#10（インボイス）・#11（二重価格）はオーナーの決定をもとに PR #23 で実装済み。ただし**運営の登録番号は「売主は誰か」が決まるまで設定しない**（→ 下の表と docs/PAYMENTS.md「インボイス」）。
...
- ビルドログに `[db:migrate] done`（0010〜0018 が当たったこと）
