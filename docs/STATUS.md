...
| #11 二重価格の運用ルール | **PR #23 に同梱**。オーナーの決定（2026-09-26、Issue #11 のコメント）: 販売の記録がある価格だけ表示・値下げ前の直近8週間の過半＋最後に売ってから2週間以内・記録がたまるまで隠す。値下げ8週間で表示をやめる（毎日の自動処理 `compare-prices`、**vercel.json に Cron を1本足した**）。マージで閉じる（`Closes #11`） |
| #18 出荷準備中のお客さまのキャンセル | **PR #23 に同梱**。オーナーの決定（2026-09-26、Issue #18 のコメント）: 準備中になった後は「キャンセルの依頼」→ 生産者が承認で全額返金・お断りでそのまま発送／複数の農家の注文は準備前の農家の分だけ取り消せる／猶予なし／規約・ガイド・最終確認画面の文言を合わせる。マージで閉じる（`Closes #18`） |
...
- ビルドログに `[db:migrate] done`（0010〜0019 が当たったこと）
...
- #11 通常価格: マージ直後は、通常価格を入れた商品でも打ち消し表示が**すべて消える**（記録がまだ無いので。決定どおり）。
  生産者の商品編集画面の規格に「販売の記録がまだないため…」と理由が出る。/admin/automation に「通常価格の表示の見直し」があり、手動実行が成功する。
  翌日以降、`variant_price_periods` に公開中の規格の行が開いている（読むだけ。本番DBへの書き込みはしない）
- #18 キャンセル: テストの注文（カードはテスト用）で
  - 準備前: マイページで「注文をキャンセル」→ 全額返金のメール（Resend 設定後）と Stripe の返金。2軒の農家の商品をまとめて買った注文なら、農家ごとの「この生産者の分をキャンセル」で片方だけ返金される
  - 生産者が「出荷準備を開始」→ マイページのボタンが「キャンセルを依頼」に変わる → 依頼すると生産者の受注一覧・出荷センターに「キャンセル依頼」が付き、
    発送済みにできない → 「承認して返金する」で返金／「お断りする」でマイページに「お断りされました」と、生産者のひとことが出る
  - ご利用ガイド・特商法の表記・注文の最終確認画面・Stripe の決済画面の文が新しい決まりになっている
- 作ったテストデータ（振込先口座・操作記録・停止したテスト用アカウント・レビューと写真・スタッフの招待とアカウント・キャンセルを試した注文を含む）を消す（→ 5節）

**オーナーの判断待ち・専門家の確認**（`owner-decision` の Issue はすべて決定済みで実装した。残りは確認と、決めていない既定。コードは決まってから）:

- #18 の残り: 新しいキャンセルの文言（`config/content.ts` の `cancellationPolicy` と利用規約 第6条）を専門家に確認（規約はまだ下書き）。
  決めていない既定: 生産者が依頼に回答しないまま出荷期限を過ぎたときの扱い（今は出荷期限のリマインドが続くだけで、自動では承認もお断りもしない）。
  スタッフへの依頼のお知らせ（今はオーナーにだけメールとお知らせが届く。スタッフは受注一覧の「キャンセル依頼」で気づく）
- #11 の残り: 決めた基準（8週間の過半・2週間以内・値下げ8週間まで）が価格表示ガイドラインに合っているかを専門家に確認（Issue #11 のコメントで「最終的には専門家に確認」としている）。
  数値は `config/catalog.ts#comparePricePolicy` だけで変えられる。販売期間は**商品が公開中（`active`）の間だけ**数え、農家の停止・お休みは見ていない（停止中はストアに出ないため）。これでよいか
...
2026-09-26 の実例（#10 インボイス）: 税額を四捨五入にする、送料を 8% にする、割引・返金を按分しない・端数を捨てる、注文明細に税率を控えない、
登録番号が空でも出す、販売手数料の税を 8% で出す、よその農園の精算を開ける、「すべて」のスタッフが登録番号を変えられる、など。
「※（軽減税率の印）を消す」は、注記の文だけを見ていたテストでは赤くならなかった。**印そのものが商品の行に出ていること**（注記と合わせて2つ以上）まで確かめるテストにした。
...
2026-09-26 の実例（#11 通常価格）: 「4週間を超える」を「以上」にする、2週間・8週間の境界を1日ずらす、2週間の条件を外す、値下げの始まりを
今の期間の始まりにする（公開し直しで8週間がリセットされる）、8週間より前の販売も数える、非公開にしても期間を閉じない、価格が変わっても新しい期間を開かない、
ストア（商品ページ・一覧・お気に入り）で生産者が入れた値をそのまま出す、保存・公開停止・アーカイブで記録を付けない、自動処理で既存の商品の記録を始めない、の18通り。
このとき**自動処理が「変わった商品」を取りこぼす**バグが見つかった（記録を付ける途中で表示が変わった商品の id を捨てていて、キャッシュが消えない）。
「変わった商品に入っているか」まで確かめるテストにしてから直した。
2026-09-26 の実例（#18 キャンセルの依頼）: 準備中でもお客さまが取り消せる、1軒でも準備中なのに注文全体を取り消せる、返金の行を押さえるときに状態を見ない、
返金の途中の出荷単位を準備・発送に進められる、依頼があるのに生産者が発送できる、運営の発送で依頼を閉じない、承認で返金しない・返金が失敗しても回答を戻さない、
お断り・依頼のお知らせを送らない、出荷担当のスタッフが回答できる、他人の注文を取り消せる・よその農園の依頼に回答できる、送り状の取り込みで依頼を見ない、
依頼を2回できる、準備前なのに依頼できる、お断りのひとことを捨てる、文言を戻す、の19通り（1つは壊し方が悪く、オブジェクトの後ろの同じキーに上書きされて
壊れていなかった。**壊したつもりの変更が本当に効いているか**も見ること）。
**赤くできなかった4つ**は、同時に押されたときだけ効く二重の守り（状態を条件にした更新）。テストのコメントに書いた（`cancel-request.test.ts` の先頭）。
...
- 壊すと**止まらなくなる**テストがある（「続きが無くなるまで読む」ループで、続きが進まなくなる壊し方など）。ループには回数の上限を付け、
  ...
  `pkill -f <テスト名>` は同じ文字列を含む自分のシェルまで止めて、後始末（`cp` で戻す）が走らなくなる（2026-09-26 に実際に起きた。同じ日に `pkill -f "vitest…"` でもう一度起きた）。
  走っているテストを止めたいときは、`ps -eo pid,args | grep vitest` で PID を見て `kill <PID>` する。止める前に、壊したファイルが残っていないか（`git diff` と退避ファイルの `cmp`）を確かめる
...
- 時間の経過で変わる処理（期限・8週間の上限など）は、関数に `now` を引数で渡す作りにし、テストでは過去・未来の時刻を渡して「その時点で呼んだ」ことにする。
  時刻はファイルの先頭で1回だけ取った基準（`const T0 = Date.now()`）から作る。呼ぶたびに `Date.now()` を取ると数ミリ秒ずれて、時刻の一致を確かめられない（`services/__tests__/price-history.test.ts`）
